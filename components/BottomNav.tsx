...
const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onTabChange, unreadCount = 0 }) => {
  const tabs = [
    { id: 'dashboard' as TabType, label: 'Home', icon: (
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-surface-card border-t border-theme-border flex items-center justify-around px-2 py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] z-50">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          ...
          className={`flex flex-col items-center justify-center flex-1 py-1 transition-all ${
            activeTab === tab.id
              ? 'text-[#00a884] scale-110'
              : 'text-theme-muted'
          }`}
        >
          {tab.icon}
          <span className="text-[10px] font-medium mt-1 uppercase tracking-tight">{tab.label}</span>
        </button>
      ))}
    </div>
  );
